...
class MosaicService {
	...
	static searchMosaics = async mosaicSearchCriteria => {
		const searchMosaics = await http.createRepositoryFactory
			.createMosaicRepository()
	...
	static getMosaicInfo = async hexOrNamespace => {
		...
		return {
			...
			expiredInBlock:
				expiredInBlock === mosaicInfo.startHeight
	...
	static getMosaicAmountViewList = async address => {
		...
		return mosaicAmountViewInfos.map(mosaicAmountViewInfo => ({
			...
			mosaicAliasNames: this.extractMosaicNamespace(
				mosaicAmountViewInfo,
				mosaicNames
			)
		}));
	 ...
	 */
	static getMosaicMetadataList = async (
		pageInfo,
		filterValue,
		hexOrNamespace
	) => {
	...
	static formatMosaicInfo = mosaicInfo => ({
		...
		relativeAmount: helper.formatMosaicAmountWithDivisibility(
			mosaicInfo.supply.compact(),
	...
	static formatMosaicAmountView = mosaicAmountView => ({
		...
		amount: helper.formatMosaicAmountWithDivisibility(
			mosaicAmountView.amount.compact(),
			mosaicAmountView.mosaicInfo.divisibility
		)
	});
	...
	static extractMosaicNamespace = (mosaicInfo, mosaicNames) => {
		...
		const names =
			0 < aliasNames.length ? aliasNames : [Constants.Message.UNAVAILABLE];
